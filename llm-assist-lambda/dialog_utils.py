...
def get_slot_values(bot_id, bot_version, locale_id, intent, slot_type):
    ...
    try:
        ...
        if len(slot_id) <= 10:
            ...
            logger.info(f"Slot type '{slot_type}' is a built-in type, skipping slot assistance")
            return None
...
def get_next_unfilled_slot(bot_id, bot_version, locale_id, intent_name, slots):
    ...
    slot_id_to_name = {
        ...
        for slot in slots_response["slotSummaries"]
    }
    ...
    for priority in sorted_priorities:
        ...
        if slot_name in slots and slots[slot_name] is None:
            logger.info(f"Will elicit slot type '{slot_name}' next based on priority order")
            return slot_name
